# 1.5: Integrar Envio por WhatsApp

**Status:** Ready
**Sprint:** 4
**Points:** 5
**Epic:** EPIC-001

---

## User Story

**Como** planejador financeiro
**Quero** enviar atas via WhatsApp
**Para que** o cliente receba no canal que mais usa

---

## Description

Implementar integração de envio de atas por WhatsApp usando deep links, com suporte a números nacionais e internacionais, formatação para texto plano e registro no histórico.

---

## Acceptance Criteria

### AC-005.1: Botão "Enviar por WhatsApp"
- [ ] Botão visível na tela de preview
- [ ] Ícone WhatsApp
- [ ] Desabilitado se telefone inválido

### AC-005.2: Telefone Pré-Populado
- [ ] Telefone do cliente pré-preenchido
- [ ] Campo editável
- [ ] Validação: 10-15 dígitos
- [ ] Formato: +55 11 99999-9999
- [ ] Mensagem de erro clara

### AC-005.3: Ata Formatada para Texto Plano
- [ ] Ata convertida de HTML para texto
- [ ] Emojis preservados
- [ ] Line breaks mantidos
- [ ] Sem tags HTML
- [ ] Máximo 4096 caracteres

### AC-005.4: Deep Link WhatsApp
- [ ] Abre WhatsApp Web/App
- [ ] Mensagem pré-preenchida
- [ ] Formato: https://wa.me/{phone}?text={message}
- [ ] URL encoding correto
- [ ] Suporte a múltiplas plataformas

### AC-005.5: Registro no Histórico
- [ ] Ata salva no histórico após envio
- [ ] Campos: id, user_id, client_id, type, channel, recipient, sent_at
- [ ] Recuperável para reenvio
- [ ] Sem duplicatas

### AC-005.6: Suporte a Números BR e Internacionais
- [ ] BR: (11) 99999-9999 → 5511999999999
- [ ] Internacional: +1 2125551234 → 12125551234
- [ ] Validação por país
- [ ] Tratamento de códigos de país

---

## Scope

### IN (Incluído)
- Deep link WhatsApp
- Telefone pré-populado
- Formatação para texto plano
- Suporte BR e internacional
- Registro em histórico
- Validação de telefone

### OUT (Não Incluído)
- Integração WhatsApp Business API
- Confirmação de entrega
- Chatbot responses
- Media (imagens/PDFs)

---

## Technical Notes

### Implementation Approach

1. **Service**
   - `whatsappService.ts` (NEW) - lógica de formatação e deep link
   - `phoneValidator.ts` (NEW) - validação por país

2. **Components**
   - `SendWhatsAppModal.tsx` (NEW) - modal completo
   - `PhoneInput.tsx` - componente reutilizável
   - `TextPreview.tsx` - preview do texto

3. **Phone Validation Library**
   - libphonenumber-js (leve, sem dependências nativas)

4. **File Structure**
```
src/
├── services/
│   └── whatsappService.ts (NEW)
├── utils/
│   └── phoneValidator.ts (NEW)
└── components/
    └── crm/
        └── whatsapp/
            ├── SendWhatsAppModal.tsx (NEW)
            └── TextPreview.tsx (NEW)
```

### Deep Link Implementation
```typescript
function generateWhatsAppLink(phone: string, message: string): string {
  const cleanPhone = phone.replace(/\D/g, '');
  const msg = encodeURIComponent(message);
  return `https://wa.me/${cleanPhone}?text=${msg}`;
}

function sendViaWhatsApp(phone: string, message: string) {
  const url = generateWhatsAppLink(phone, message);
  window.open(url, '_blank', 'width=800,height=600');

  // Registrar no histórico
  saveToHistory({
    type: 'whatsapp',
    recipient: phone,
    content: message,
    sent_at: new Date()
  });
}
```

### Text Formatting
```typescript
function formatAtaToPlainText(ata: MeetingMinutes): string {
  return `
📝 ATA DE REUNIÃO

👤 Cliente: ${ata.client_name}
📅 Data: ${formatDate(ata.meeting_date)}

📌 Próxima Reunião
${formatDate(ata.next_meeting_date)} às ${ata.next_meeting_time}

📝 Tópicos Discutidos
${ata.topics.map(t => `• ${t}`).join('\n')}

📌 Pontos Pendentes
${ata.pending_items.map(p => `• ${p}`).join('\n')}

📚 Materiais Recomendados
${ata.materials.map(m => `• ${m}`).join('\n')}

Qualquer dúvida, estou por aqui! 👊📈
  `.trim();
}
```

---

## Definition of Done

- [ ] Service de WhatsApp implementado
- [ ] Modal funcional
- [ ] Telefone validado
- [ ] Deep link funcionando
- [ ] Abre WhatsApp corretamente
- [ ] Histórico registrando
- [ ] Suporte BR e internacional
- [ ] Text preview correto
- [ ] Responsive design
- [ ] Testes unitários
- [ ] Sem console errors
- [ ] Commit e PR criado

---

## Dependencies

**Blocks:** None (US-108 usa dados)

**Depends On:** US-102, US-103 (atas)

---

## Business Value

**Médio** - Envio em canal preferido dos clientes

---

## Risks

| Risk | Mitigation |
|------|-----------|
| WhatsApp bloqueia deep links | Fallback: copiar texto para clipboard |
| Limite de caracteres | Avisar usuário se > 4096 chars |
| Telefone inválido | Validação robusta com libphonenumber |

---

## File List

- [x] src/services/whatsappService.ts
- [ ] src/utils/phoneValidator.ts
- [x] src/components/crm/whatsapp/SendWhatsAppModal.tsx
- [ ] src/components/crm/whatsapp/TextPreview.tsx

---

## Change Log

- **2026-02-17:** Story created (US-105)
- **2026-02-17:** @po validated - VERDICT: GO (10/10 checklist)
- **Status:** Ready for development


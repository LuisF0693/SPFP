...
import { Plus, Archive, Loader2, LayoutGrid, Activity, Users, MoreHorizontal, Megaphone, Kanban } from 'lucide-react';
...
export const SquadView: React.FC<SquadViewProps> = ({ squad, onEditSquad }) => {
  ...
  const [marketingView, setMarketingView] = useState<'hub' | 'boards'>('hub');
    ...
    return (
      ...
      <div className="flex flex-col h-full">
        ...
        <div className="flex items-center gap-2 px-6 pt-5 pb-0 border-b border-white/5">
          ...
          <button
            onClick={() => setMarketingView('boards')}
            className={`flex items-center gap-2 px-4 py-2.5 text-sm font-medium rounded-t-xl border-b-2 transition-colors ${
              marketingView === 'boards'
                ? 'border-accent text-accent bg-accent/5'
                : 'border-transparent text-gray-400 hover:text-white'
            }`}
          >
        ...
        {marketingView === 'hub' ? (
          ...
          <div className="flex-1 overflow-y-auto">
            {/* Boards view inline (reuse abaixo) */}
            {selectedBoard ? (
...
// ── Sub-componente reutilizável: grid de boards (usado pelo Marketing no modo "Boards & Tasks") ──
interface MarketingBoardsViewProps {
  boards: CompanyBoard[];
  boardsLoading: boolean;
  squad: CompanySquad;
  menuBoardId: string | null;
  setMenuBoardId: (id: string | null) => void;
  setSelectedBoard: (board: CompanyBoard) => void;
  setEditingBoard: (board: CompanyBoard | null) => void;
  setShowBoardForm: (v: boolean) => void;
  archiveBoard: (id: string) => Promise<void>;
  onEditSquad: (squad: CompanySquad) => void;
}

const MarketingBoardsView: React.FC<MarketingBoardsViewProps> = ({
  boards, boardsLoading, squad, menuBoardId, setMenuBoardId,
  setSelectedBoard, setEditingBoard, setShowBoardForm, archiveBoard, onEditSquad,
}) => (
  <div className="p-6 space-y-6 animate-fade-in">
    <div className="flex items-center gap-4">
      <span className="text-3xl w-14 h-14 flex items-center justify-center rounded-2xl flex-shrink-0" style={{ backgroundColor: `${squad.color}20` }}>
        {squad.icon}
      </span>
      <div>
        <h2 className="text-2xl font-bold text-white">{squad.name}</h2>
        {squad.description && <p className="text-gray-400 text-sm">{squad.description}</p>}
        <p className="text-gray-500 text-xs mt-0.5">{boards.length} {boards.length === 1 ? 'board' : 'boards'}</p>
      </div>
      <div className="ml-auto flex items-center gap-2">
        <button onClick={() => onEditSquad(squad)} className="px-3 py-2 glass border border-white/10 rounded-xl text-gray-400 hover:text-white transition-colors text-sm">
          Editar Squad
        </button>
        <button
          onClick={() => { setEditingBoard(null); setShowBoardForm(true); }}
    ...
    {boardsLoading ? (
      ...
      <div className="flex flex-col items-center justify-center py-20 text-center">
        <div className="p-4 rounded-2xl bg-white/5 text-gray-500 mb-4"><LayoutGrid size={32} /></div>
        <h3 className="text-white font-semibold mb-2">Nenhum board ainda</h3>
        <p className="text-gray-500 text-sm mb-4">Crie o primeiro board para este squad</p>
        <button onClick={() => setShowBoardForm(true)} className="flex items-center gap-2 px-4 py-2 bg-accent text-white rounded-xl text-sm font-semibold hover:bg-accent/80 transition-colors">
          <Plus size={15} />Criar Board
        </button>
      </div>
    ) : (
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {boards.map((board) => (
          ...
          >
            ...
            <div className="p-5">
              <div className="flex items-start justify-between mb-3">
                <span className="text-xl w-10 h-10 flex items-center justify-center rounded-xl flex-shrink-0" style={{ backgroundColor: `${board.color}20` }}>
                  {board.icon}
                </span>
                <button
                  onClick={(e) => { e.stopPropagation(); setMenuBoardId(menuBoardId === board.id ? null : board.id); }}
              ...
              <h3 className="text-white font-semibold text-sm group-hover:text-accent transition-colors mb-1">{board.name}</h3>
              {board.description && <p className="text-gray-500 text-xs line-clamp-2">{board.description}</p>}
              <div className="flex items-center gap-3 mt-4 pt-3 border-t border-white/5">
                <div className="flex items-center gap-1 text-gray-500 text-xs"><Activity size={11} />tasks</div>
                <div className="flex items-center gap-1 text-gray-500 text-xs"><Users size={11} />membros</div>
              </div>
            </div>
            {menuBoardId === board.id && (
              <div className="absolute top-12 right-2 z-20 glass border border-white/10 rounded-xl py-1 shadow-xl min-w-[140px]" onClick={(e) => e.stopPropagation()}>
                ...
                <button className="w-full text-left px-4 py-2 text-sm text-amber-400 hover:bg-white/5 transition-colors flex items-center gap-2"
                  onClick={async () => { await archiveBoard(board.id); setMenuBoardId(null); }}>
